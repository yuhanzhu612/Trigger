...
class Brpu extends Module{
  ...
  when (jmp_packet.valid) {
    bht(bht_waddr) := Cat(jmp_packet.taken.asUInt(), bht_wrdata(BhtWidth - 1, 1))
  }
  ...
  when (jmp_packet.valid) {
    pht_wstate := MuxLookup(pht_wstate, default_state(), Array(
      0.U -> Mux(jmp_packet.taken, 1.U, 0.U),   // strongly not taken
      1.U -> Mux(jmp_packet.taken, 2.U, 0.U),   // weakly not taken
      2.U -> Mux(jmp_packet.taken, 3.U, 1.U),   // weakly taken
      3.U -> Mux(jmp_packet.taken, 3.U, 2.U)    // strongly taken
    ))
  }
  ...
  when (jmp_packet.valid && jmp_packet.taken) {
    btb(btb_waddr).valid := true.B
    btb(btb_waddr).tag := btb_tag(jmp_packet.inst_pc)
    btb(btb_waddr).target := jmp_packet.target
  }

  when (jmp_packet.valid && jmp_packet.mispredict) {
    pred_br := false.B
    pred_pc := Mux(jmp_packet.taken, jmp_packet.target, jmp_packet.inst_pc + 4.U)
  } .otherwise {
    when (pht_rdirect) {
